"""
F6 验证：活力四方案死代码已清理，仅保留 current 口径
- 正常：活力X 的攻击出手 = 1 + X（+ 狂暴1），与现行口径一致
- 边界：无活力/仅狂暴/活力+狂暴 三种组合
...
def test_normal_vitality_current():
    """正常：活力X 按 current 语义 +X 出手"""
    m = make_monster({"name":"羊","ac":1,"ap":1,"hp":60,"dw":{"活力":2},"region":"龙心谷"})
    assert get_monster_attack_actions(m, {"活力"}) == 1 + 2
    assert get_monster_attack_actions(m, set()) == 1
    assert get_monster_attack_actions(m, {"狂暴"}) == 2  # 狂暴+1
    assert get_monster_attack_actions(m, {"活力","狂暴"}) == 1 + 2 + 1

def test_boundary_no_vitality_vs_zero():
    """边界：未激活活力时不受 X 影响；活力0 视为无"""
    m = make_monster({"name":"无","ac":1,"ap":1,"hp":60,"dw":{},"region":"扭曲都市"})
    # 无道纹时即使传活力激活集也应为 1（通用回退）
    assert get_monster_attack_actions(m, set()) == 1
    # 狂暴单独
    assert get_monster_attack_actions(m, {"狂暴"}) == 2
    # 活力激活但怪物面板无该道纹（极边界）：仍按 +X 取值，但 X 不存在时取 0 -> 回退 1
    # 实际实现取 x_value 时若无道纹会 KeyError，此处验证行为为不崩溃且为 1+0
    # 为此构造有道纹但 X=0 的情况
    m0 = make_monster({"name":"零活力","ac":1,"ap":1,"hp":60,"dw":{"活力":0},"region":"龙心谷"})
    assert get_monster_attack_actions(m0, {"活力"}) == 1
...
def test_error_invalid_huoli_mode_rejected():
    ...
    m = make_monster({"name":"羊","ac":1,"ap":1,"hp":60,"dw":{"活力":3},"region":"龙心谷"})
    # 模拟外部污染：即使写入属性，也不应被读取（函数内无分支）
    bs.HUOLI_MODE = "burst"  # type: ignore[attr-defined]
    try:
        assert get_monster_attack_actions(m, {"活力"}) == 4
    finally:
