...
# ========================================================================
# 7. 吞骸龙胃 × 【缄默】（DM 裁定 2026-09-18：吞噬窗口属"由[命零]触发的效果"）
# ========================================================================

def _kill(engine, victim, source):
    """走统一伤害管线击灭（命零判定与死亡上下文都在里面）。"""
    return engine.combat._apply_hostile_damage(victim, 99, source=source, ctx={
        "timing": "player_action", "source": "杀伐", "source_type": "daowen",
        "actor": source, "target": victim, "mechanic": "damage", "subtype": "daowen",
        "amount": 99, "tags": {"daowen"},
    })


def _devour_engine(dbsuffix):
    engine = _new_engine(dbsuffix=dbsuffix)
    _with_heart(engine)
    engine.execute_action("unlock_dragon_trait", {"trait": "吞骸龙胃"})
    engine.execute_action("battle_start", {})
    engine.execute_action("round_start", {})
    victim = Entity(name="骸骨", entity_type="怪物", blood_limit=30, current_hp=30)
    engine.state.enemies.clear()
    engine.state.enemies.append(victim)
    return engine, victim


def test_dragon_stomach_blocked_for_a_corpse_that_died_under_silence():
    """封禁期内命零的尸体：窗口没开过，不可吞噬，且不给回复"""
    from engine.daowen import DaoWenEngine
    engine, victim = _devour_engine("devour_silenced")
    player = engine.state.player

    engine.combat.apply_daowen_effect("缄默", DaoWenEngine.calculate_qianmo(2), player, player)
    hp_before = player.current_hp
    detail = _kill(engine, victim, player)

    assert detail["died"] is True and victim.is_alive is False
    assert "silenced_death" in victim._death_ctx["tags"]
    r = engine.execute_action("devour_monster", {"monster_ref": "enemy:0"})
    assert r["success"] is False
    assert "缄默" in r["error"]
    assert player.current_hp == hp_before, "被拒绝就不得给[回复12]"


def test_dragon_stomach_still_works_for_a_corpse_that_died_before_silence():
    """对照：缄默之前命零的尸体窗口已经开过，之后才封禁也照样可吞（看死亡那一刻）"""
    from engine.daowen import DaoWenEngine
    engine, victim = _devour_engine("devour_after_silence")
    player = engine.state.player
    player.current_hp = 10

    _kill(engine, victim, player)
    assert "silenced_death" not in victim._death_ctx["tags"]
    engine.combat.apply_daowen_effect("缄默", DaoWenEngine.calculate_qianmo(2), player, player)
