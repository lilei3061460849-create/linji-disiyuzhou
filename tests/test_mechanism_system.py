...
# ------------------------------------------------------------------ 【缄默】默认封禁
# DM 裁定 2026-09-18：ENTITY_DIED 机制一律视为「由[命零]触发的效果」，封禁由总线默认执行。
# 原因：Condition 是闭包、不可内省，"每条机制自己记得挂 death_not_silenced()" 必然漂移——
# 忘挂的机制会静默逃过【缄默】，而且没有任何测试会红。默认生效 + 显式退出把失效方向反过来。


def _died_event(silenced: bool) -> CombatEvent:
    return CombatEvent(event_type=CombatEventType.ENTITY_DIED, battle_no=1, round_no=1,
                       actor_name="P", target_name="M", data={"silenced": silenced})


def test_entity_died_mechanism_is_silenced_by_default():
    """默认封禁：不挂任何条件的命零机制，在封禁期内也不会被分发。"""
    bus = TriggerBus()
    fired: list = []
    probe = Mechanism(name="探针命零反应", when=Trigger.event(CombatEventType.ENTITY_DIED),
                      effect=lambda ctx, targets: fired.append(ctx.event.data["silenced"]))
    bus.register(probe)

    bus.dispatch(_died_event(silenced=True), combat=None)
    assert fired == [], "封禁期内的命零事件不得分发给任何 ENTITY_DIED 机制"

    bus.dispatch(_died_event(silenced=False), combat=None)
    assert fired == [False], "非封禁的命零事件必须照常分发（门禁只吃 silenced 标记）"


def test_entity_died_mechanism_can_opt_out_of_silence_explicitly():
    """退出封禁是显式的：ignores_silence=True 的机制在封禁期照常触发。"""
    bus = TriggerBus()
    fired: list = []
    ledger = Mechanism(name="探针纯账面", when=Trigger.event(CombatEventType.ENTITY_DIED),
                       effect=lambda ctx, targets: fired.append(1), ignores_silence=True)
    bus.register(ledger)
...
def test_silence_gate_only_touches_entity_died():
    """门禁只作用于 ENTITY_DIED：别的事件即便带 silenced 标记也照常分发。"""
    bus = TriggerBus()
    fired: list = []
    probe = Mechanism(name="探针伤害反应", when=Trigger.event(CombatEventType.DAMAGE_APPLIED),
                      effect=lambda ctx, targets: fired.append(1))
    bus.register(probe)

    bus.dispatch(CombatEvent(event_type=CombatEventType.DAMAGE_APPLIED,
                             data={"silenced": True}), combat=None)
    assert fired == [1]


def test_production_registry_silence_opt_out_needs_a_reason():
    """生产注册表里退出封禁的命零机制必须逐个登记理由，防止悄悄漏封。"""
    # 需要退出封禁时在此登记「机制名 → 为什么它不属于由[命零]触发的效果」。
    allowed_opt_out: dict[str, str] = {}
    offenders = sorted(
        m.name for m in MECHANISMS.all()
        if m.when.kind == "event" and m.when.key == CombatEventType.ENTITY_DIED
        and m.ignores_silence and m.name not in allowed_opt_out
    )
    assert not offenders, f"这些命零机制退出了【缄默】封禁却没登记理由：{offenders}"


def test_jiaohheifasi_is_subject_to_the_default_gate():
    """真机制回归：焦黑发丝是唯一在册的命零机制，且未退出默认封禁。"""
    mechanism = MECHANISMS.get("焦黑发丝")
    assert mechanism is not None
    assert mechanism.when.kind == "event"
    assert mechanism.when.key == CombatEventType.ENTITY_DIED
    assert mechanism.ignores_silence is False
