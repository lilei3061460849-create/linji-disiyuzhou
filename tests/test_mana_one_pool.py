...
def test_battle_start_grants_full_pool():
    ...
    e = _engine("start", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    assert p.mana_limit == 22
    r = begin_battle(e)
    assert r["success"], r
    assert p.current_mana == p.mana_limit == 22, p.current_mana


def test_relic_gain_stacks_over_full_pool():
    """边界：遗物获得叠在满池上，允许超过[法限]（反向禁区：不得加 clamp）。"""
    e = _engine("relic", blood_points=6, speed_points=8, mana_points=11)
    e.state.relics.append(Relic(name="折速法印", effect="[战始]可疲惫X获得6X法力"))
    p = e.state.player
    begin_battle(e, relic_choices={"折速法印": {"use": True, "x": 4}})
    assert p.current_mana == p.mana_limit + 24 == 46, p.current_mana
...
def test_round_start_does_not_refill():
    ...
    e = _engine("refill", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    begin_battle(e)
    assert p.spend_mana(17) is True
    assert p.current_mana == 5
    _finish_round(e)
    r = begin_round(e)
    assert r["success"], r
    assert [x for x in r["result"].get("effects", [])
            if x.get("type") == "mana_refill"] == [], "一池制：不得有回填条目"
    assert p.current_mana == 5, f"回始不得回填，实{p.current_mana}"


def test_round_end_keeps_leftover_mana():
    """正常路径：回终与敌回终都不清空，剩余法力跨回合保留。"""
    e = _engine("keep", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    begin_battle(e)
    assert p.spend_mana(17) is True
    r = _finish_round(e)
    ...
    assert p.current_mana == 5, f"回终不得清空，实{p.current_mana}"
    begin_round(e)
    assert p.current_mana == 5, f"下一回始仍是5，实{p.current_mana}"


def test_pool_never_refills_across_many_rounds():
    """边界：连走三回合，池子只减不增（除非有遗物/道纹显式给）。"""
    e = _engine("many", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    begin_battle(e)
    assert p.current_mana == 22
    _finish_round(e)
    for i in range(3):
        assert p.spend_mana(5) is True, f"第{i + 1}回合应还能付 5"
        _finish_round(e)
        begin_round(e)
        assert p.current_mana == 22 - 5 * (i + 1), (i, p.current_mana)
    assert p.spend_mana(8) is False, "只剩 7，付不起 8"
...
def test_battle_end_restores_mana_like_speed():
    ...
    e = _engine("end", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    begin_battle(e)
    assert p.spend_mana(19) is True
    p.current_speed = 1
    assert (p.current_mana, p.current_speed) == (3, 1)
    _clear_enemies(e)
    r = e.execute_action("battle_end", {})
    assert r.get("success"), r
    assert p.current_mana == p.mana_limit == 22, f"战终应复原法力，实{p.current_mana}"
    assert p.current_speed == p.speed_limit == 8, f"战终应复原速度，实{p.current_speed}"


def test_battle_end_does_not_grant_mana_to_non_reincarnator():
    """错误输入/对照：朋友不是轮回者，战终不给它复原法力。"""
    e = _engine("friend", blood_points=6, speed_points=8, mana_points=11)
...
def test_gouhun_doubles_cost_until_it_expires():
    ...
    e = _engine("gouhun", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
    begin_battle(e)
    p.add_status(StatusEffect(name="勾魂", value=1, remaining_rounds=2, source="寄骨蝇"))
    assert p.spend_mana(5) is True
    assert p.current_mana == 12, f"5 点应翻倍扣 10，实剩 {p.current_mana}"
    assert p.spend_mana(7) is False, "翻倍后需 14，只剩 12 → 付不起"
    assert p.current_mana == 12, "付不起时不得扣费"

    _finish_round(e)
    _finish_round(e)
    assert not p.has_status("勾魂"), "持续X=2 走完应自然到期"
    assert p.spend_mana(5) is True
    assert p.current_mana == 7, f"到期后按原值扣，实剩 {p.current_mana}"


def test_gouhun_does_not_touch_mana_on_apply():
    """边界：勾魂只改消耗倍率，挂上/到期都不动当前法力。"""
    e = _engine("gouhun_apply", blood_points=6, speed_points=8, mana_points=11)
    p = e.state.player
