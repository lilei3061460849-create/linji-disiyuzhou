"""已迁移到声明层的机制（当前 9 个：加害、龙鳞、帮派令、衰败、畸变·结算、
...
迁移协议（迁移前后必须同时满足）：
  ...
  2. priority 保持原值或按旧代码位置固化：加害=20、龙鳞=30（伤害加减区）；
     衰败=20、洞察·结算=30、狂暴·标记=50、畸变·标记=60
     （自愈=10 已于 2026-09-18 随【自愈】重做为主动道纹而整体移除；
      其余 priority 一律不重排，衰败=20 顺延为回始循环第一位）
...
SHUAIBAI = Mechanism(
    ...
    # 旧位置=回始效果循环第二位（原自愈=10 之后、洞察之前）；自愈机制已于 2026-09-18
    # 随道纹重做移除，本 priority 不重排，因此衰败现在是回始循环第一位
    priority=20,
)
