...
`RuleSync()`默认同时跟踪 README.md、死者之书.md、物品索引.md、副本索引.md 四份正文（`法术索引.md` 是展示层文档，不参与规则抽取）；不得再从README推断已经迁出的物品或副本怪物。
...
├── ai_player.py         # 统一 AI 玩家入口（开局/事件/战斗/校验）
...
【畸变·结算】（priority=10，血限走统一入口，lethal=True）——经
