...
│                        #   已迁移机制：加害（原 JiahaiHook）、龙鳞（原 LonglinHook）、自愈/衰败（原 round_start 内嵌块）、帮派令（原 process_relics 战始 if）。新机制优先写声明层，勿回核心管线加 if。
...
回始原位分发；【帮派令】（priority=10）——经 `_dispatch_phase(Phase.BATTLE_START)` 战始原位
分发，条件复用通用 `relic_active("帮派令")`。旧类/旧 if 均已删除。护栏：`validator.check_migrated_mechanism_guards()` 禁止已迁移机制
