...
│                        #   已迁移机制：加害（原 JiahaiHook）、龙鳞（原 LonglinHook）。新机制优先写声明层，勿回核心管线加 if。
...
priority=30）——均经 `MechanismHookAdapter` 在 Hook 分发路径原位执行，旧类已删除。护栏：`validator.check_migrated_mechanism_guards()` 禁止已迁移机制
