...
5. **诚实门禁** — 未实装的机制（员工/龙心/部分遗物/赌命消灾等）在 API 层如实标记 `unavailable` 并拒绝结算，**绝不返回看似成功的空壳结果**
...
| `round_start` / `round_end` | ✅ | 回始/回终（法力、格挡、持续效果、衰败/清算/逼债/活血等钩子；皮衣/负岳索/储能电池/皮衣试穿修饰） |
| `battle_end` | ✅ | 战终（碎片、清理、冷却推进、叛变检查、冠冕触发；悬赏/博彩/三倍盘/果实花/植入倒计时） |
| `random_number` | ✅ | 提交随机数 |
...
python tests/test_engine.py # 16项真实结算测试
