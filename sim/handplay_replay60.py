#!/usr/bin/env python3
"""重跑验证驱动器（2026-09-15）：把第 2 次轮回（白夜）录下的决策序列，
原样灌进 60 点面板的怪物池，检验"35 点时代赢了 7 场的那套打法"在 60 点下还能不能过。

只做三件事——执行、记录、存档；本文件不替 AI 做任何决策：
  * 每个普通动作按 trace 里录到的参数原样重交；
  * __auto_attack__ / __monster_phase__ 用录到的逐击/逐 actor 选择重交，token 现取；
  * 首个被拒的步骤就是"同一决策序列在这里走不通"的分界点，须由 AI 实时重新决策。

用法（同主驱动器）：
    python3 sim/handplay_replay60.py --new --steps-file /tmp/slice.json
...
SESSION_DIR = os.path.join(ROOT, "data", "handplay_replay60")
...
def _monster_phase(engine: GameEngine, policy: dict | None = None,
                  token: str | None = None, prep_only: bool = False,
                  choices_raw: list | None = None) -> dict:
    ...
    if choices_raw is not None:
        # 重跑模式：直接重交 trace 里录到的逐 actor 选择（合法性仍由引擎校验）
        choices = choices_raw
        print(f"    ▶ 重放录到的怪物侧选择：{json.dumps(choices, ensure_ascii=False)[:400]}")
    else:
        try:
...
def main() -> None:
    ...
    ap.add_argument("--stop-on-reject", action="store_true",
                    help="重跑模式：一旦出现被拒步骤就停在该分界点（后续步骤不再提交）")
    args = ap.parse_args()
    ...
    def _new_rows_failed(before: int) -> bool:
        """本步新写入的流水里是否出现 success=False（重跑分界点）。"""
        rows = []
        with open(TRACE_PATH, encoding="utf-8") as fh:
        ...
        if bad:
            print(f"    ⛔ 重跑分界点：本步出现被拒提交 {[r['action'] for r in bad]}")
        return bool(bad)
    ...
    try:
      for i, step in enumerate(steps, 1):
        ...
        if action == "__auto_attack__":
            ...
            if args.stop_on_reject and _new_rows_failed(_rows_before):
                break
            continue
        if action == "__monster_phase__":
            ...
            _monster_phase(engine, pol or {}, token=params.get("token"),
                           ...
                           choices_raw=params.get("choices_raw"))
            if args.stop_on_reject and _new_rows_failed(_rows_before):
                break
            continue
        if action == "__round__":
