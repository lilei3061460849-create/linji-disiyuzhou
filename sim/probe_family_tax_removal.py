"""机制验证探针：删除怪物「家族税」后的四条口径（2026-09-18 用户令）。

**这是机制验证，不是轮回数据**（铁律6：脚本轮回数据视为虚假数据）——只用来确认
引擎在删税后仍按预期结算，不得当成平衡样本引用。跑法：

    .venv/bin/python sim/probe_family_tax_removal.py

四条读数（2026-09-18 实跑）：
  1. prepare 的可负担 X 上限：必中/减速/飞行＝9（异变45，崩解线封顶）、变形＝9（法限9）、
     自愈＝7（`X_LIMITS["冷却"]=7` 才是紧约束，删税前家族税算出的 9 从来没生效过）；
  2. 发动自愈：异变 **0** 层（删税前是 5X）、冷却 7 场、已损生命 175% 回复照常；
  3. 发动必中9：异变仍 **45** 层（崩解 45/50）——证明删税**救不回异变型自爆**，
     那是必中自身正文代价「异变5X」的结果，解法仍是战术选 X 接线（报告 Q10）；
...
print("=== 3) 发动必中9（自身代价＝异变5X）：家族税删了也照样 45 层 ===")
