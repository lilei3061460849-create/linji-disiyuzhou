...
STALE_PHRASES: list[tuple[tuple[str, ...], str]] = [
    ...
    # 波及口径（2026-09-19 用户裁定 D10 选案 A）：现行＝「所有**由你**挂上波及效果的目标」＋
    # 「[目标]选自己时自己也保留一份」。旧句把「拥有波及效果」读成「任何带波及状态的角色」，
    # 与实现的**单向性**不符（`_wave_targets` 只认 `source==施法者` 的标记），且掩盖了
    # 「自施时自己被剔出扩散名单」这个已修掉的缺陷。
    (("同时作用于所有拥有波及效果",),
     "【波及】2026-09-19 改口径＝只认「由你挂上」的标记；自施时自己也保留一份（见 报告.md 6.21）"),
    ...
    (("爆裂", "受到伤害后"), "【爆裂X】反噬时点＝受到伤害**前**（combat_hooks.BaolieHook.on_before_damage，priority 40）"),
    (("变形", "攻击力与攻击次数互换"), "【变形X】2026-09-17 用户令重做＝当前速度↔当前法力互换（钳掉的部分凭空消失）"),
    (("超频", "使自身速度"), "【超频X】2026-09-17 用户令＝目标由发动方自由指定（实现一直是给 target 加速）"),
    (("定型", "无法增加"), "【定型X】实现挡增减两个方向（_panel_keys 同含 attack_boost 与 attack_reduction）"),
    (("钱袋",), "已删遗物钱袋"),
    (("双倍计入",), "癌变双倍计入已删除"),
