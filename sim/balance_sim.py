...
# ===== 活力（2026-08-17 全局裁定；裁定⑫其余候选已归档删除 2026-08-11） =====
# 现行（engine侧）：活力X → 所有角色出手次数+X，持续∞；异变5X仅在首次发动时支付。
# 本模拟器镜像只覆盖怪物自身攻击出手+X（发动方视角与全局等价）；
# 玩家侧与其他怪物的+X尚未建模，活力相关模拟数据须按 B6 重测后才可引用。
...
def get_monster_attack_actions(m, activated):
    """怪物攻击出手数（现行口径）：1 + 活力X(已激活) + 狂暴1

    引擎2026-08-17全局裁定后，怪物侧+X由自身活力状态驱动；
