#!/usr/bin/env python3
"""
引擎驱动战报生成器：全程用GameEngine API驱动（开局遗物/局外/道纹/法术/怪物回合/遗物/雕塑/增生/还债
    ...
    for n in range(1, 8):
        ...
        for rnd in range(1, 30):
            ...
            if vp:
                ...
                won = True; break
        if not won and not [e for e in engine.state.enemies if e.is_alive]:
            won = True
        if not won:
        ...
        log.append(f"  [战终] 碎片→{shards}{'(含雕塑/增生/还债等移出'+str([x['name'] for x in rem])+')' if rem else ''}，HP{engine.state.player.current_hp}\n")
