...
# 特殊注记（引擎机制，非数值）
NOTES = {
    "波及": "目标由两阶段决策显式提交恰好X个（怪物侧prepare枚举候选dodge_target_options，玩家侧use_daowen的dodge_targets）；X上限＝场上当前角色总数，波及不能选自己，可标记目标不足X时由发动方自己把X选小；一个可标记目标都没有时怪物prepare不给出该道纹。你发动的道纹对已标记目标同时生效，数值平分。",
