...
用法：
    python3 sim/handplay_cycle_20260914.py --steps '[{"action":"...","params":{...}}]'
    python3 sim/handplay_cycle_20260914.py --steps-file /tmp/steps.json
    python3 sim/handplay_cycle_20260914.py --steps '[{"action":"__state__"}]'
"""
...
SESSION_DIR = os.path.join(ROOT, "data", "handplay_20260914")
...
def _engine() -> GameEngine:
    ...
    return GameEngine(
        ...
        rng_seed=914,
        sealed_candidate_path=os.path.join(SESSION_DIR, "sealed_candidate.json"),
...
def _build_monster_choices(prepared: dict, policy: dict) -> list[dict]:
    ...
    for actor in prepared.get("actors") or []:
        ...
        def _hit(_):
            ...
            sc = {k: {sp["spell_name"]: {"use": False} for sp in (spells.get(k) or [])}
                  for k in ("before", "after")}
            for k in ("damage_after", "life_before"):
                if spells.get(k):
                    sc[k] = {sp["spell_name"]: {"use": False} for sp in spells[k]}
