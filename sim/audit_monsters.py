...
现行唯一口径（2026-09-11：预算并入属性点，README正文）：
  轮回者同口径：1属性点=6血限，2属性点=1速限=1法限；怪物攻次/攻力分别映射速限/法限，一阶预算35，道纹单独配额。
= panel_cost = ⌈血限/6⌉ + 2×攻击次数 + 2×攻击力 ≤ 35。
（此前35点面板使用旧的攻击次数²计价；本次改为轮回者同口径，并按
  hp=6×(35-2×攻击次数-2×攻击力)重算36只面板，道纹串不变。）
