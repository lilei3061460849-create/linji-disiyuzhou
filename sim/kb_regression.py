...
PROBES = [
    ...
    dict(id="C2-03", cat=2, q="怪物道纹发动代价（只按各道纹自身代价）",
         gold=[["怪物发动道纹只按该道纹自身代价支付", "怪物发动只付该道纹自身代价"],
               ["自愈＝冷却X", "自愈＝【冷却X】"]]),
    dict(id="C2-05", cat=2, q="道纹变化（退化降X实战注意）", gold=["退化"]),
    dict(id="C2-06", cat=2, q="资源限制：法力支付/冷却锚点", gold=["冷却"]),
    dict(id="C2-07", cat=2, q="波及X上限（现行裁定：场上当前角色总数）",
         gold=[["场上当前角色总数", "场上角色总数"]], stale=["自适应降X"], layer="SHARED"),
    # 三、副本规则
    ...
    dict(id="C3-05", cat=3, q="全力怪池占比报告纪律（2/12 条目）",
         gold=["罪孽2/12", "扭曲2/12", "龙心1/12"]),
    dict(id="C3-06", cat=3, q="死斗封存规则存在性", gold=["死斗", ["擂主", "封存"]]),
    dict(id="C3-07", cat=3, q="净化X 机制（现行代价＝消耗2X，旧5X已改）", gold=["净化X：消耗2X"]),
    ...
    dict(id="S5-01", cat=5, q="波及旧语义（过滤/自适应降X皆已删）",
         stale=["不进入 daowen_options", "自适应降X"], gold=["场上当前角色总数"]),
    ...
    # 旧 C2-04（怪物道纹递增 +2×阶级）已废止 → 转 stale 探针：现行口径是「面板不写X、
    # 发动方自选，上限只受[法限]或代价限制」（规则正文·怪物准则9）。
    dict(id="S5-07", cat=5, q="怪物道纹递增（已废止机制，不得以当前规则出现）",
         stale=["累加+2×副本阶级", "X+2×副本阶级", "道纹递增+"],
         gold=["面板不写X", "发动方自选"]),
    # 六、机器契约（文档校验器/锚点标题）
